...
class TemplateBuilder(object):
    ...
    def __init__(self, **kwargs):
        ...
        self._termcomponent_names = set() #added
        ...
        self._end_conditions = dict() #added
        ...
        self._term_complementary_states = set()#added
        ...
        #added:
        endcomponents = kwargs.pop('endconcentrations', dict())
        if isinstance(endcomponents, dict):
            for k, v in endcomponents.items():
                self._termcomponent_names.add(k)
                self._end_conditions[k] = v
        else:
            raise RuntimeError('endconcentrations must be an dictionary component_name:end_condition')
        ...
        termextra_states = kwargs.pop('termextra_states', dict())
        if isinstance(termextra_states, dict):
            for k, v in termextra_states.items():
                self._term_complementary_states.add(k)
                self._end_conditions[k] = v
        else:
            raise RuntimeError('Terminal extra states must be an dictionary state_name:end_condition')
    ...
    def add_mixture_component(self, *args):
        ...
        if len(args) == 1:
            ...
            raise RuntimeError('Mixture component data not supported. Try str, float')
#addbc:
    def add_term_component(self, *args):
        """Add a component (reactive or product) to the model.
        ...
        Args:
            ...
            param2 (float): terminal concentration condition.   

            param1 (dict): Map component name(s) to terminal concentrations value(s).
        ...
        if len(args) == 1:
            ...
            if isinstance(input, dict):
                for key, val in input.items():
                    if not isinstance(val, numbers.Number):
                        raise RuntimeError('The terminal condition must be a number. Try str, float')
                    self._termcomponent_names.add(key)
                    self._end_conditions[key] = val
            else:
                raise RuntimeError('Terminal component data not supported. Try dict[str]=float')
        elif len(args) == 2:
            name = args[0]
            end_condition = args[1]

            if not isinstance(end_condition, numbers.Number):
                raise RuntimeError('The second argument must be a number. Try str, float')

            if isinstance(name, six.string_types):
                self._termcomponent_names.add(name)
                self._end_conditions[name] = end_condition
            else:
                raise RuntimeError('Terminal component data not supported. Try str, float')
        else:
            raise RuntimeError('Terminal component data not supported. Try str, float')
    ...
    def add_complementary_state_variable(self, *args):
        ...
        if len(args) == 1:
            ...
            print(len(args))
            raise RuntimeError('Complementary state data not supported. Try str, float')
    def add_term_complementary_state_variable(self, *args):
        """Add an extra state variable to the model.
        ...
        if len(args) == 1:
            ...
            if isinstance(input, dict):
                for key, val in input.items():
                    if not isinstance(val, numbers.Number):
                        raise RuntimeError('The init condition must be a number. Try str, float')
                    self._term_complementary_states.add(key)
                    self._term_conditions[key] = val
            else:
                raise RuntimeError('Complementary state data not supported. Try dict[str]=float')
        elif len(args) == 2:
            name = args[0]
            ...
            if isinstance(name, six.string_types):
                self._term_complementary_states.add(name)
                self._term_conditions[name] = end_condition
            else:
                raise RuntimeError('Complementary state data not supported. Try str, float')
        else:
            print(len(args))
    ...
    def create_pyomo_model(self, start_time, end_time):
        ...
        pyomo_model.term_components = Set(initialize=self._termcomponent_names)#added
        ...
        pyomo_model.term_complementary_states = Set(initialize=self._term_complementary_states)#added
        pyomo_model.states = pyomo_model.mixture_components | pyomo_model.complementary_states
        pyomo_model.termstates = pyomo_model.term_components | pyomo_model.term_complementary_states #added
        ...
        #addterm:
        pyomo_model.term_conditions = Param(pyomo_model.termstates, initialize=self._end_conditions, mutable=True)
        ...
        def rule_term_conditions(m, k):
            end = end_time
            #print(m.term_components.value)
            if k in m.term_components:
                #print(self._end_conditions[k])
                return m.Z[end, k] == self._end_conditions[k]
            else:
                return m.X[end, k] == self._end_conditions[k]

        pyomo_model.term_conditions_c = Constraint(pyomo_model.termstates, rule=rule_term_conditions)
        if pyomo_model.term_conditions_c:
            for v in pyomo_model.term_conditions.keys():
                if not isinstance(v, str):
                    print("v is not str")
                    sys.exit()
                #print(v)
                #print(pyomo_model.init_conditions_c[v])
                pyomo_model.init_conditions_c[v].deactivate()
                print("Initial condition for", v, "is deactivated due to number of DOF")
        #pyomo_model.term_conditions_c.pprint()
        #pyomo_model.init_conditions_c.pprint()
        ##########
